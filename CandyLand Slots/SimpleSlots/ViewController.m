...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if ((nb_shown_lobby % ADS_INTERSTITIAL_ON_LOBBY_FREQUENCY) == 0) {
        ...
        //        NSLog(@"Call display AppLovin interstitial");
    }
#endif
}
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    if([connectState isEqual:@"updatecoins"]){
        ...
        if([response isEqual:@""] || [downloadData count] < 8){
            ...
            //NSLog([downloadData description]);
...
- (void)viewDidLoad{
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad){
        ...
        [btnDownloadCancel setFont:ft20];
        [btnDownloadNow setFont:ft20];
    }else{
        ...
        [displayCoins setFont:ft10];
        [btnDownloadCancel setFont:ft10];
        [btnDownloadNow setFont:ft10];
    }
...
-(void)manageServer:(NSString *)where{
    ...
    //NSLog(@"fbid: %@", [CommonUtilities decryptString:@"efbid"]);
    //NSLog(@"fbemail: %@", [CommonUtilities decryptString:@"efbemail"]);
    //NSLog(@"fbname: %@", [CommonUtilities decryptString:@"efbname"]);
